...
Some characteristics of srun that will mean more
...
This example is run on Tinkercliffs (TC).

The context here is "embarrassingly parallel" computing.

#### Inputs

Please copy the contents of the files below into files
on the cluster, using the same names.
...
2. Delete the line `#SBATCH --cpus-per-task=12` and in its place insert
   `#SBATCH --cpus-per-task=1`.
3. Submit this job by typing `sbatch sbatch_slow.03.slurm`
4. When it completes, you can see the execution time by 
   typing `sh run.timing.sh <SLURM_JOB_ID>`.



## Use of srun and GNU parallel