...
These checks are often done with IF statements.
...
this must be defined if the incremental value is defined).
Note that other options can be
added, but these will be system-specific, and will not be covered here.
The default values for both the start and incremental values are 1.
The sequence will be built according to these rules,
starting at the first value, and continuing until the next value created would be greater than
the last value.
...
The numbers are:  3, 7, 11, 15, 19, 23, 27, 31, and 35.

So the reverse sequence is:

`seq 35 -4 3`

Note: because `seq` starts from the first number given, you have to use
...
- In the range `seq 0 3`, both the 0 and the 3 are included in the loops
(which is different from C/C++, Java, Python, ...)
...
1) Please do the same using the C-style notation loop.

2) Please adapt the C-style notation loop to run through the array in reverse order.
