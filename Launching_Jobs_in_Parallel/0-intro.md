# Welcome to the Launching Job in Parallel on ARC Workshop

Date: April 3, 2026
Presented by: Matthew Brown
[Spring 2026 ARC Workshop Schedule](https://github.com/AdvancedResearchComputing/Workshops/blob/main/README.md)

## Outline
0. (5 min) [Welcome](./0-intro.md)
1. (5 min) [Slurm job options overview](./1-slurm.md)
2. (10 min) [Common patterns for launching](./2-patterns.md)
3. (10 min) [MPI launching with `srun` or `mpirun`](./3-mpi.md)
4. (10 min) [GNU `parallel` overview](./4-parallel.md)
5. (10 min) [`srun` + `parallel` ](./5-srun_parallel.md)

## Acknowledgement
I have pulled some content developed by others, particularly Chris Kuhlman.